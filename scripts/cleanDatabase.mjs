...
async function main() {
  ...
  // 1. Wipe all existing document records for aniket.shinde21450@gmail.com
  console.log(`🧹 Wiping all old document records for ${USER_EMAIL}...`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    console.log(`[${i + 1}/${files.length}] [Category: ${categoryId}] Storing: ${filename}...`);
    ...
    // Insert clean metadata row into Postgres
    ...
    if (dbErr) {
      console.error(`  ❌ Postgres insert error:`, dbErr.message);
    } else {
      console.log(`  ✅ Stored cleanly as ${categoryId}`);
      successCount++;
    }
  }

  console.log(`\n🎉 Database Cleaned & Synchronized! Exactly ${successCount} unique documents stored.`);
}
